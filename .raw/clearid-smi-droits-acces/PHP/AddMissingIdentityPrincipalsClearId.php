...
$photo_name = '';
$igg_list = array();
$debug_filename = realpath(dirname(__FILE__)) . '\AddMissingIdentityPrincipalsClearId_' . date("Ymd_His") . '.log';
...
//		print_r($response); echo "\n";
	...
	//echo "\n" . $url_principalservice . '/api/v3/accounts/' . $account_id . '/identityPrincipals/' . $igg['identityId'];
			curl_setopt_array($curl2, array(
				CURLOPT_URL => $url_principalservice . '/api/v3/accounts/' . $account_id . '/userPrincipals/' . rawurlencode($igg['email']),
				CURLOPT_RETURNTRANSFER => true,
				CURLOPT_SSL_VERIFYHOST => false,
				CURLOPT_SSL_VERIFYPEER => false,
				CURLOPT_ENCODING => '',
  ...
  "principalState": "Active"
			...
			$msg = "\n -> roles : " . implode(", ", $response2['roles']);
			$msg .= "\n -> principalState : " . $response2['principalState'];
			echo $msg;
			if ($DEBUG_FILE) file_put_contents($debug_filename, $msg, FILE_APPEND);
		}
