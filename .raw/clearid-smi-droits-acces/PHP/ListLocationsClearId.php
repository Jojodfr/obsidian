...
// CSV header
$csv = "siteId;locationId;name;visibility" . PHP_EOL;
...
//Recupération des locations (secteurs)
$num = 1;
...
curl_setopt_array($curl8, array(
	CURLOPT_URL => $url_locationservice . '/api/v3/accounts/' . $account_id . '/locations',
	CURLOPT_RETURNTRANSFER => true,
	CURLOPT_SSL_VERIFYHOST => false,
	CURLOPT_SSL_VERIFYPEER => false,
	CURLOPT_ENCODING => '',
...
$locations = $response8['results'];

/*  Pour chaque identité ClearID */
foreach ($locations as $location) {

	$msg = "\n\nLocation " . $num . " : " . $location['locationId'] . " " . $location['name'];
	$msg .= "\nsiteId  : " . $location['siteId'];
	$msg .= "\nvisibility  : " . $location['visibility'];
	echo $msg;
	if ($DEBUG_FILE) file_put_contents($debug_filename, $msg, FILE_APPEND);

	$cas_a = array($location['siteId'], $location['locationId'], $location['name'], $location['visibility']);
